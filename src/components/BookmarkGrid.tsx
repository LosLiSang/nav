...
function BookmarkItem({
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className="group relative flex h-11 cursor-grab select-none items-center gap-3 rounded-xl border border-neutral-200/70 px-3 shadow-sm transition hover:shadow-md active:cursor-grabbing"
    ...
    >
      ...
      />
      <span className="min-w-0 flex-1 truncate text-sm text-neutral-800">{bookmark.title}</span>
      <div className="absolute right-1 hidden items-center gap-1 group-hover:flex">
        <button
          ...
          className="rounded bg-white/90 px-1.5 py-0.5 text-xs text-neutral-600 shadow"
        >
          编辑
        </button>
        <button
          ...
          className="rounded bg-white/90 px-1.5 py-0.5 text-xs text-red-500 shadow"
        >
          删除
        </button>
      </div>
